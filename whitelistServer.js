var WhiteListServer = [];
WhiteListServer.push(412262889156771842); //Official Server
WhiteListServer.push(356891225715900427); //Rised-bot Server
WhiteListServer.push(368837055993741323); //Benpoke Server
WhiteListServer.push(378217520236068864); //Sloper39 Server
WhiteListServer.push(412287827787907072); //Downell FR Server
WhiteListServer.push(368461834929307648); //Laeti Zep Server
WhiteListServer.push(373919412325056513); //Skelever & Ramzer Server
WhiteListServer.push(413838785835696129); //Grogeek Server
WhiteListServer.push(405092181519368192); //WindowsMe User Server
WhiteListServer.push(389494042620329984); //Tard0sTV Server (Polyvalence)
WhiteListServer.push(288001034083893248); //Tachyro Server
WhiteListServer.push(307954526152032256); //Discrod Banane (Enoctes) Server
WhiteListServer.push(281529034477797376); //PLfightX Server
//Debug => console.log("Whitelist: " + WhiteListServer)

exports.WhiteListServer = WhiteListServer;